% Example 1: Estimate the price of a European call option on a stock
% The stock price is modeled by a geometric Brownian motion.

function [v, se] = call_option_euro(s0, r, sigma, T, K, n)
...
y = s0*exp((r-0.5*sigma^2)*T + sigma*sqrt(T)*z); % stock price at T
x=exp(-r*T)*max(y-K,0);  % payoff of the option at T
